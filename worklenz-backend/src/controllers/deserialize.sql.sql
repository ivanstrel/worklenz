         ...
         appsumo_data AS (SELECT tod.owner_id,
                                 EXISTS(SELECT 1
                                        ...
                                        WHERE lcc.redeemed_by = tod.owner_id
                                          ...
                                          AND lcc.is_refunded = FALSE) AS is_ltd,
                                 (SELECT COUNT(*)::INT
                                  FROM licensing_coupon_codes lcc
                                  WHERE lcc.redeemed_by = tod.owner_id
                                    AND lcc.is_redeemed = TRUE
                                 ...
                                 (SELECT CASE
                                             WHEN (SELECT COUNT(*)::INT
                                                   ...
                                                   WHERE lcc.redeemed_by = tod.owner_id
                                                     AND lcc.is_redeemed = TRUE
                                                     AND lcc.is_refunded = FALSE) >= 5 THEN TRUE
                                             ELSE FALSE
                                             END)                      AS appsumo_business_eligible
...
alter function deserialize_user(uuid) owner to cdsadmin;
